...
- [ ] **Escenario 5 (Migración error_codes):** Tabla `error_codes` con campos: `id (bigserial PK)`, `code (varchar)`, `application_id (bigint FK → applications.id)`, `name (varchar)`, `description (text nullable)`, `severity (enum nullable)`, `default_role_id (bigint nullable)`, `timestamps`. Unique constraint: `(code, application_id)`.
