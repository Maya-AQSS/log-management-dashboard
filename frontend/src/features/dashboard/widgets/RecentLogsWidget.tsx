...
function RecentLogsWidget() {
 ...
 {[1, 2, 3].map((n) => (<div
 key={n}
 className="h-10 bg-outline-variant rounded-lg animate-pulse"
 />
 ))}
 </div>
 );
 ...
 return (<ul className="flex flex-col gap-2 overflow-auto h-full" role="list">
 {logs.map((log) => (<li key={log.id}>
 <Link
 to={`/logs/${log.id}`}
 className="flex items-start gap-2 px-2 py-2 rounded-md hover:bg-surface transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-primary"
 >
