...
const inputClass =
'w-full bg-surface-container-low border border-outline rounded-md text-on-surface px-3 py-2 text-sm outline-none focus:border-primary transition-colors';
const fieldLabelClass =
'text-on-surface-variant text-xs font-medium uppercase tracking-wide';
...
function writeFiltersToUrl(filters: ArchivedLogsFiltersState,
 sortBy: ArchivedLogsSortKey | null,
 sortDir: SortDir | null,
 page: number,
): URLSearchParams {
...
function toApiFilters(filters: ArchivedLogsFiltersState,
 sortBy: ArchivedLogsSortKey | null,
 sortDir: SortDir | null,
 page: number,
...
export function ArchivedLogsPage() {
 ...
 const { filters, sortBy, sortDir, page } = useMemo(() => parseFiltersFromUrl(searchParams),
 [searchParams],
 );
 ...
 const updateFilters = useCallback((patch: Partial<ArchivedLogsFiltersState>) => {
 ...
 const sortState: SortState | null = useMemo(() => (sortBy && sortDir ? { columnId: sortBy, direction: sortDir } : null),
 [sortBy, sortDir],
 );
 ...
 {state.status ==='error' && (<Alert tone="danger" className="mt-4">{t('listLoadError', { message: state.error })}
 </Alert>
 )}

 {state.status ==='loading' && !pagination && (<div className="mt-4 rounded-lg border border-outline bg-surface-container-low p-6 text-center text-sm text-on-surface-muted">
 {t('detail.loading')}
 </div>
 )}
