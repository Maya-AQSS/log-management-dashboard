...
export function ErrorCodeDetailPage() {
 ...
 {state.status ==='error' && (<Alert tone="danger" className="mt-4">No se pudo cargar el código de error: {state.error}
 </Alert>
 )}

 {state.status ==='loading' && !ec && (<div className="mt-4 rounded-lg border border-outline bg-surface-container-low p-6 text-center text-sm text-on-surface-muted">
 Cargando…
 </div>
 )}

 {ec && (<div className="mt-4 space-y-4">
 ...
 {editing && saveError && (<Alert tone="danger" className="mt-4">{saveError}</Alert>
 )}
