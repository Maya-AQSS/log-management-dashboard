...
export function ErrorCodesPage() {
 ...
 const updateFilters = useCallback((patch: Partial<ErrorCodesFiltersState>) => {
 ...
 className="inline-flex items-center bg-primary text-on-primary border-primary hover:bg-primary-hover hover:border-primary-hover px-4 py-1.5 rounded-md text-sm font-semibold transition-colors cursor-pointer border shadow-sm"
 >
 Nuevo código
 </Link>
 }
 ...
 {state.status ==='error' && (<Alert tone="danger" className="mt-4">No se pudieron cargar los códigos de error: {state.error}
 </Alert>
 )}
