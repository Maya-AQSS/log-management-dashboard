<x-layout>
    <div class="flex items-start justify-between gap-3">
        <a
            href="{{ route('error-codes.show', $errorCode->id) }}"
        ...
        <div class="text-center">
            <h1 class="text-xl font-semibold">{{ __('error_codes.edit_title') }}</h1>
            <p class="text-base text-gray-500">{{ __('error_codes.edit_subtitle') }}</p>
        </div>

        <div class="w-20"></div>
    </div>

    <form method="POST" action="{{ route('error-codes.update', $errorCode->id) }}" class="mt-4 bg-white border rounded-2xl p-4 space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-base">
            <div>
                ...
                >
                    ...
                    @foreach($applications as $id => $name)
                        <option value="{{ $id }}" @selected((string) old('application_id', $errorCode->application_id) === (string) $id)>{{ $name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                >
                    <option value="">-</option>
                    <option value="critical" @selected(old('severity', $errorCode->severity) === 'critical')>{{ __('severity.critical') }}</option>
                    <option value="high" @selected(old('severity', $errorCode->severity) === 'high')>{{ __('severity.high') }}</option>
                    <option value="medium" @selected(old('severity', $errorCode->severity) === 'medium')>{{ __('severity.medium') }}</option>
                    <option value="low" @selected(old('severity', $errorCode->severity) === 'low')>{{ __('severity.low') }}</option>
                    <option value="other" @selected(old('severity', $errorCode->severity) === 'other')>{{ __('severity.other') }}</option>
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <textarea
                    ...
                    rows="4"
                    class="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-base shadow-sm focus:border-[#5b3853] focus:outline-none focus:ring-2 focus:ring-[#5b3853]/20"
                >{{ old('description', $errorCode->description) }}</textarea>
