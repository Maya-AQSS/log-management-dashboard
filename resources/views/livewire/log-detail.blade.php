    ...
    @if($source === 'archived_log')
        ...
        >
            ...
            <div class="flex flex-col items-center gap-2 text-center md:gap-3">
                <h1 class="text-3xl font-bold leading-tight text-slate-900 md:text-4xl dark:text-slate-100">
                    ...
                    @can('delete', $archivedLog)
                        @if(!$editingUrlTutorial)
                            ...
                            >
                                ...
                                <button
                                    type="submit"
                                    class="rounded-full bg-red-600 px-3 py-1.5 text-base font-semibold text-white shadow-sm hover:bg-red-500"
                                >
...
<div class="mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-950 dark:shadow-none">
    @if($source === 'log')
        ...
        <div class="grid grid-cols-1 gap-4 text-sm md:grid-cols-2">
            <div>
                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ __('logs.detail.id') }}</div>
                <div class="mt-0.5 text-slate-800 dark:text-slate-200">{{ $log->id }}</div>
            </div>

            <div>
                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ __('logs.table.application') }}</div>
                <div class="mt-0.5 text-slate-800 dark:text-slate-200">{{ $log->application?->name ?? '—' }}</div>
            </div>

            <div>
                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ __('logs.table.severity') }}</div>
                <div class="mt-0.5 text-slate-800 dark:text-slate-200">
                    <x-severity-badge :severity="$log->severity" />
                </div>
            </div>

            <div>
                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ __('logs.table.status') }}</div>
                <div class="mt-0.5 text-slate-800 dark:text-slate-200">
                    @if($log->resolved)
                    ...
                    @else
                        <span class="text-slate-700 dark:text-slate-300">{{ __('logs.status.unresolved') }}</span>
                    @endif
                </div>
            </div>

            <div>
                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ __('logs.table.error_code') }}</div>
                <div class="mt-0.5 text-slate-800 dark:text-slate-200">{{ $log->errorCode?->code ?? '—' }}</div>
            </div>

            <div>
                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ __('logs.table.created_at') }}</div>
                <div class="mt-0.5 text-slate-800 dark:text-slate-200">
            ...
            <div>
                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ __('logs.detail.file') }}</div>
                <div class="mt-0.5 break-all text-slate-800 dark:text-slate-200">{{ $log->file ?? '—' }}</div>
            </div>

            <div>
                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ __('logs.detail.line') }}</div>
                <div class="mt-0.5 text-slate-800 dark:text-slate-200">{{ $log->line ?? '—' }}</div>
            </div>

            <div class="md:col-span-2">
                <div class="mb-1 font-semibold text-slate-900 dark:text-slate-100">{{ __('logs.table.message') }}</div>
                <div
                    class="max-h-64 overflow-y-auto rounded-lg border border-slate-200 bg-slate-50 p-3 text-slate-900 whitespace-pre-wrap break-words dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 md:max-h-96"
                >
                    {{ $log->message ?? '—' }}
                </div>
            </div>
            ...
            <div class="md:col-span-2">
                <div class="mb-1 font-semibold text-slate-900 dark:text-slate-100">{{ __('logs.detail.metadata') }}</div>
                @if($metadataJson !== null)
                    <pre
                        class="max-h-64 overflow-y-auto rounded-lg border border-slate-200 bg-slate-50 p-3 font-mono text-xs text-slate-900 whitespace-pre-wrap break-all dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 md:max-h-96"
                    >{{ $metadataJson }}</pre>
                @else
    ...
    @else
        <div class="grid grid-cols-1 gap-4 text-sm md:grid-cols-2">
            <div>
                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ __('logs.detail.id') }}</div>
                <div class="mt-0.5 text-slate-800 dark:text-slate-200">{{ $archivedLog->id }}</div>
            </div>

            <div>
                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ __('logs.table.application') }}</div>
                <div class="mt-0.5 text-slate-800 dark:text-slate-200">{{ $archivedLog->application?->name ?? '—' }}</div>
            </div>
            ...
            <div>
                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ __('logs.table.error_code') }}</div>
                <div class="mt-0.5 text-slate-800 dark:text-slate-200">{{ $archivedLog->errorCode?->code ?? '—' }}</div>
            </div>
